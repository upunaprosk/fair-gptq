    ...
    def quantize(
        ...
        if self.qcfg.mock_quantization or (self.fail_safe and self.fwd_counter == 0):
            ...
            for i1 in range(0, self.columns, blocksize):
                ...
                Hcopy1 = None
                if self.log_hessian_sensitivity and self.H_x01 is not None:
                    Hcopy1 = self.H_x01[i1:i2, i1:i2]
                ...
                for i in range(count):
                        ...
                        if not self.qcfg.static_groups:
                            if (i1 + i) % self.qcfg.group_size == 0:
                                self.quantizer.find_params(
                                    W[
                                        :,
                                        (i1 + i) : (i1 + i + self.qcfg.group_size),
                                    ],
                                    weight=True,
                                )
                    ...
                    q = self.quantizer.quantize(w.unsqueeze(1)).flatten()
                    Q1[:, i] = q
                    if Hinv1 is not None and d is not None:
                        Losses1[:, i] = (w - q) ** 2 / d**2
                        err1 = (w - q) / d
                    ...
                    if self.log_hessian_sensitivity:
                        # Column-level sensitivity: ||w - q|| * diag(H or H_x01)
                        norm_diff = torch.norm(W1[:, i] - Q1[:, i], p=2)
                        if self.alpha and Hcopy1 is not None:
                            d_sens = Hcopy1[i, i]
                        else:
                            d_sens = d if d is not None else torch.tensor(
                                0.0, device=W.device
                            )
                        d_times_norm = d_sens * norm_diff
            ...
            # Save sensitivities if requested
            if self.log_hessian_sensitivity and sensitivities_H is not None:
                ...
                try:
                    ...
                    if len(all_indices) != self.columns:
                        raise ValueError(
                            f"Expected {self.columns} sensitivities_H, got {len(all_indices)}"
                        )
                    if len(unique_indices) != self.columns:
                        dupes = [
                            x for x in all_indices if all_indices.count(x) > 1
                        ]
                        raise ValueError(
                            f"Duplicate column indices found in sensitivities_H: {set(dupes)}"
                        )
                except ValueError as e:
                    log.info(f"sensitivities_H check failed: {e}")
                    raise
                else:
                    log.info(
                        "sensitivities_H index check passed. Proceeding to save."
                    )
                full_name = getattr(self, "full_name", self.name)
                os.makedirs("./logs", exist_ok=True)
                file_name = f"./logs/{full_name}_{self.alpha}_sensitivity_H.txt"
                try:
                    with open(file_name, "w") as f:
                        for idx, val in sensitivities_H:
                            f.write(f"Column {idx} {val}\n")
                except Exception as e:
                    log.warn(f"Failed to write sensitivities_H for `{full_name}`: {e}")

        if Hinv is not None:
            del Hinv
            if H2inv is not None:
                del H2inv
            if self.nsamples != 0:
        ...
        del Losses
        del self.H
        if self.H_x01 is not None:
            del self.H_x01
            self.H_x01 = None

        group_size = self.qcfg.group_size if self.qcfg.group_size != -1 else self.columns

        if self.qcfg.static_groups and self.qcfg.desc_act and perm is not None:
        ...
        elif self.qcfg.act_group_aware and final_perm is not None and global_perm is not None:
