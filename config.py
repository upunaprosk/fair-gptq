...
META_FIELD_ALPHA = "alpha"

META_FIELD_IGNORE_BIAS = "ignore_bias"

META_FIELD_SELECT_LAYER = "select_layer"

META_FIELD_SUM_HESSIANS = "sum_hessians"

META_FIELD_DELTA_W = "log_delta_w"

META_FIELD_LOG_HESSIAN_SENSITIVITIES = "log_delta_w"
...
class QuantizeConfig():
    ...
    # alpha for the unbias component, by default no unbias, equal to gptq
    alpha: float = field(default=0.0)

    # for ablation studies ignore_bias component, default no
    ignore_bias: str = field(default="")

    # for ablation studies select layer matrices only, default no
    select_layer: str = field(default="")

    # whether to consider the second order in the hessian
    sum_hessians: int = field(default=0)

    # weather to log delta w relative change
    log_delta_w: int = field(default=0)

    # weather to log H sensitivities
    log_hessian_sensitivity: int = field(default=0)
            ...
            for layer, layer_dict in self.dynamic.items():
                for key, value in layer_dict.items():
                    if key == "bits" and value not in fields_info[0].metadata["choices"]:
                        ...
                        raise ValueError(
                            "QuantizeConfig: `group_size` must in the value set of `[-1, 16, 32, 64, 128]`.")
    ...
    def from_quant_config(cls, quantize_cfg, format: str = None):
        ...
        if format_auto_inferred:
            log.info(
                f"QuantizeConfig: `{FORMAT_FIELD_JSON}` is missing from the quantization configuration and is automatically inferred to {normalized[FORMAT_FIELD_CODE]}")
